...
export function Wardrobe({ item, selected, invalid }: Props) {
  ...
  const trim = finish.trim;

  return (
    <group>
      <mesh position={[0, h / 2, 0]} castShadow receiveShadow>
        <boxGeometry args={[w, h, d]} />
        <meshStandardMaterial color={finish.color} map={finish.map ?? undefined} roughness={0.68} />
      </mesh>

      <mesh position={[0, h + crownH / 2, 0]} castShadow>
        <boxGeometry args={[w + 1.5, crownH, d + 1]} />
        <meshStandardMaterial color={trim} map={finish.map ?? undefined} roughness={0.55} />
      </mesh>
      ...
      {[-1, 1].map((side) => (
        <group key={side}>
          <mesh position={[(side * doorW) / 2, h / 2, dz]} castShadow>
            ...
            <meshStandardMaterial color={finish.color} map={finish.map ?? undefined} roughness={0.72} />
          </mesh>
      ...
      <mesh position={[0, h / 2, dz]} castShadow>
        ...
        <meshStandardMaterial color={trim} map={finish.map ?? undefined} roughness={0.6} />
      </mesh>

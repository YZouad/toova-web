...
import { lampPartsFromSize } from '../furniture/lampGeometry';
...
function buildLampGroup(): THREE.Group {
  ...
  const stem = new THREE.Mesh(
    new THREE.CylinderGeometry(stemR, stemR, stemH, 12),
    new THREE.MeshStandardMaterial({ color: '#6b6357', roughness: 0.45, metalness: 0.2 }),
  );
  stem.position.y = baseH + stemH / 2;
  group.add(stem);
  const shade = new THREE.Mesh(
    new THREE.CylinderGeometry(shadeW * 0.25, shadeW * 0.5, shadeH, 16, 1, true),
    new THREE.MeshStandardMaterial({
      color: '#f2e6d4',
      roughness: 0.85,

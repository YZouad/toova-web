...
class USDZExporter {
	...
	async parseAsync( scene, options = {} ) {
		...
		for ( const id in textures ) {
			...
			// WebGL-based decompress is unavailable in the edge runtime.
			...
			if ( ! isDrawableTextureImage( texture.image ) ) {
...
function isDrawableTextureImage( image ) {
	...
	if ( typeof image.width === 'number' && image.width > 0 &&
		...
		( image.data instanceof Uint8ClampedArray || image.data instanceof Uint8Array ) ) {
...
function imageToCanvas( image, flipY, maxTextureSize ) {
	...
	if ( ( typeof HTMLImageElement !== 'undefined' && image instanceof HTMLImageElement ) ||
		...
		( typeof OffscreenCanvas !== 'undefined' && image instanceof OffscreenCanvas ) ||
		( typeof ImageBitmap !== 'undefined' && image instanceof ImageBitmap ) ||
		( image && image.data instanceof Uint8ClampedArray && image.width > 0 && image.height > 0 ) ) {
		...
		const imageType = image && image.constructor ? image.constructor.name : typeof image;
		const dataBytes = image && image.data ? image.data.byteLength : 0;
		throw new Error(
			'THREE.USDZExporter: No valid image data found. Unable to process texture. ' +
			`type=${ imageType } size=${ image && image.width }x${ image && image.height } data=${ dataBytes }`
		);

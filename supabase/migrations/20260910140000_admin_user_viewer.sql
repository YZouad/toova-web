-- Last-active heartbeat + admin user viewer (rooms, models, analytics slot).
-- Admins get SELECT-only access so they can open a user's rooms/models without
-- inheriting write rights.
...
-- Own-row heartbeat. Does not bump profiles.updated_at (that is for profile edits).
...
COMMENT ON FUNCTION public.touch_own_last_active() IS
  'Signed-in user: stamp profiles.last_active_at, throttled to once per 45s. No-op if no profile row.';

-- Admin read-only access (OR'd with existing owner/public policies).
...
-- Users list: every profile, with last_active derived from heartbeat + activity.
...
COMMENT ON FUNCTION public.get_admin_user_item_totals() IS
  'Admin-only: every profile with room/model counts and derived last_active_at.';

-- Condensed user record + rooms + models + analytics envelope (series/extras reserved).
...
COMMENT ON FUNCTION public.get_admin_user_overview(uuid) IS
  'Admin-only: condensed user profile, rooms, models, and analytics envelope (series/extras reserved for custom analytics).';

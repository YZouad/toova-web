...
-- Small monochrome PNG stored beside palette thumbnails in model-files.

ALTER TABLE public.furniture_catalog
  ADD COLUMN IF NOT EXISTS silhouette_path text;

-- Allow public browse/sign of silhouette assets for public catalog rows.
...
-- Include lamp + silhouette_path via catalog model_url join.
...
-- Return silhouette path on delete so clients can purge storage.
